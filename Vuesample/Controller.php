...
class [UNAME]Controller extends Controller {
    ...
    public function index(List[UNAME]Request $request) { 
        ...
        if($only->authorize()){
            $this->data['lists'] = [Controller_Relation]->where('created_by', user()->id)->paginate(25);
        }
    ...
    public function destroy(Delete[UNAME]Request $request){
        ...
        try {
            ...
            \DB::rollback();                        
            return errorResponse('Error while deleting [LNAME]. Try again latter');
        }
        \DB::commit();
        ...
        $this->data['list_data'] = $model->list_data();
        return successResponse(Lang::get('[TNAME].delete_message'),$this->data);
    }
}
